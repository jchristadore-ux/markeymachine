...
**Monitoring & Telegram control.** A background watchdog alerts you on Telegram in real time when a worker crashes/stalls (and can auto-restart it); a token-protected `/health` JSON and an admin log viewer expose every worker's status and recent logs; [`tools/railway_logs.py`](tools/railway_logs.py) can pull raw Railway logs. The same operator Telegram bot accepts **commands** (`/status`, `/logs`, `/analyze`, `/format`, `/set`, `/pause`, `/restart`, with `/confirm` before any change) to diagnose and adjust the safe trading levers (format + allowlisted params) from your phone. See [`MONITORING.md`](MONITORING.md) and the consolidated [`RAILWAY_SETUP.md`](RAILWAY_SETUP.md).
